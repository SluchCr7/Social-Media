      ...
      <div className="grid grid-cols-1 w-full lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 p-4 rounded-2xl bg-gradient-to-br from-gray-900/60 to-gray-900/40 border border-gray-800">
          ...
          {topPosts.length ? topPosts.map(p => {
            ...
            return (
              <div key={p._id} className="flex items-center justify-between p-3 rounded-xl bg-gray-800/30 mb-2">
                <div className="flex-1 pr-4">
                  <div className="truncate text-sm font-medium">{p.text || p.caption || '—'}</div>
                  <div className="text-xs text-gray-400 mt-1">{p.likes?.length || 0} likes • {p.comments?.length || 0} comments</div>
                </div>
