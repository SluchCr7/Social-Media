...
const AdminUsersPage = () => {
  const { users, deleteUser, updateAccountStatus, makeUserAdmin } = useAuth();
  ...
  return (
    <div className="min-h-screen p-8 bg-lightMode-bg dark:bg-darkMode-bg w-full max-w-[1200px] mx-auto">
      <h1 className="text-3xl font-bold mb-6 text-lightMode-text2 dark:text-darkMode-text2">Users Management</h1>
      ...
      <div className="space-y-6">
        {currentUsers.map(user => (
          <div key={user._id} className="bg-white dark:bg-darkMode-card p-6 rounded-2xl shadow-md hover:shadow-xl hover:scale-105 transition-transform border border-gray-200 dark:border-gray-700">
            
            {/* User Info */}
            <div className="flex items-center gap-4 mb-4">
              <div className={`w-[50px] h-[50px] rounded-full overflow-hidden ring-2 ${user.isAdmin ? 'ring-blue-500 dark:ring-blue-400' : 'ring-transparent'}`}>
                <Image
              ...
              <div>
                <p className="font-bold text-lg text-lightMode-text2 dark:text-darkMode-text2">{user.username}</p>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {user.isAdmin && <span className="px-2 py-1 text-xs bg-blue-100 text-blue-700 rounded-full" title="Admin"><FiUserCheck/></span>}
              {user.accountStatus === 'banned' && <span className="px-2 py-1 text-xs bg-red-100 text-red-700 rounded-full" title="Banned">Banned</span>}
              {user.isAccountWithPremiumVerify && <span className="px-2 py-1 text-xs bg-blue-100 text-blue-700 rounded-full" title="Verified"><HiBadgeCheck/></span>}
              {user.isPrivate && <span className="px-2 py-1 text-xs bg-gray-100 text-gray-700 rounded-full" title="Private">Private</span>}
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                title={user.accountStatus === 'banned' ? 'Unban this user' : 'Ban this user'}
              >
          ...
          <div className="flex justify-center mt-6 gap-2">
            {Array.from({ length: totalPages }, (_, i) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg ${currentPage === i + 1 ? 'bg-blue-500 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200'} hover:bg-blue-500 hover:text-white transition`}
              >
                {i + 1}
              </button>
            ))}
