  ...
  return (
    <div className="fixed inset-0 w-full h-full flex items-center justify-center bg-[#050505] overflow-hidden">
      ...
      <motion.div
        ...
        className="relative z-10 w-full max-w-5xl grid md:grid-cols-2 bg-white/5 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] shadow-2xl overflow-hidden min-h-[650px]"
      >
        ...
        {/* Right Side: Form */}
        <div className="flex flex-col justify-center p-10 md:p-14 bg-black/20 overflow-y-auto custom-scrollbar">
          <div className="md:hidden text-center mb-8">
            <h2 className="text-3xl font-bold text-white mb-2">{t('Create Account')}</h2>
            <p className="text-gray-400">{t('Join us securely')}</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5 max-w-md mx-auto w-full">
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-gray-400 uppercase tracking-wider ml-1">{t('Username')}</label>
              <div className="relative group">
                <FiUser className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-blue-400 transition-colors" size={20} />
                <input
                  type="text"
                  name="username"
                  ...
                  required
                  className="w-full pl-12 pr-4 py-4 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:bg-white/10 focus:border-blue-500/50 transition-all font-medium"
                />
              </div>
            </div>
            ...
            {/* Email */}
            <div className="space-y-1">
              <label className="text-xs font-bold text-gray-400 uppercase tracking-wider ml-1">{t('Email Address')}</label>
              <div className="relative group">
                <FiMail className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-blue-400 transition-colors" size={20} />
                <input
                  type="email"
                  name="email"
            ...
            <div className="space-y-1">
              ...
              <div className="relative group">
                <FiLock className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-blue-400 transition-colors" size={20} />
                <input
                  ...
                  onChange={handleChange}
                  required
                  className="w-full pl-12 pr-12 py-4 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:bg-white/10 focus:border-blue-500/50 transition-all font-medium"
                />
                <button
                  type="button"
            ...
            <div className="flex items-start gap-3 mt-2 group cursor-pointer" onClick={() => setAgreeTerms(!agreeTerms)}>
              <div className={`w-5 h-5 rounded border border-white/20 flex items-center justify-center transition-all mt-0.5 ${agreeTerms ? 'bg-blue-500 border-blue-500' : 'bg-transparent'}`}>
                {agreeTerms && <FiCheck className="text-white text-xs" />}
              </div>
              <p className="text-sm text-gray-400 leading-snug">
                {t('I agree to the')}{' '}
                <Link href="/Pages/Terms" className="text-blue-400 hover:text-blue-300 transition-colors underline decoration-blue-400/30 hover:decoration-blue-300">
            ...
            <p className="text-center text-gray-400 text-sm mt-6">
              ...
              <Link href="/Pages/Login" className="text-blue-400 hover:text-blue-300 font-bold transition-colors">
                {t('Login')}
              </Link>
            </p>
          </form>
