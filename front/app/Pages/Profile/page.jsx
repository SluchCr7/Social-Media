...
//             ${userData?.stories?.length > 0 ? 'bg-gradient-to-tr from-pink-500 via-yellow-400 to-purple-600' : 'bg-transparent'}
//           `}
...
const ProfilePage = () => {
  ...
  // تحقق إذا كان الزائر تابع للـ user أو هو نفسه
  const isFollower = userData?.followers?.includes(user?._id) || user?._id === userData?._id
      ...
      <div className="w-full max-w-7xl mx-auto min-h-screen bg-lightMode-bg dark:bg-darkMode-bg text-lightMode-text dark:text-darkMode-text px-4 sm:px-6 lg:px-8 py-6 grid grid-cols-1 gap-6">
        ...
        <div className="flex flex-col items-center lg:items-start justify-start gap-6">
          ...
          >
            ...
            {/* Change button visible only for owner */}
          ...
          {/* Actions: always visible for owner */}
          {user?._id === userData?._id && (
            <div className="flex gap-3 flex-wrap">
              <button
                onClick={() => setUpdate(true)}
          ...
          {/* Stats and Info: hide if not follower */}
          {isFollower ? (
            <>
              <div className="flex flex-col sm:flex-row justify-center lg:justify-start items-center gap-6 sm:gap-10 mt-4 w-full">
                <StatBlock label="Posts" value={userData?.posts?.length} />
            ...
            <p className="mt-4 text-gray-500 text-center sm:text-left">
              This account is private. Follow to see posts, stats, and detailed information.
            </p>
          )}
        </div>
        ...
        <div className="flex flex-col gap-6 w-full">
          ...
          {isFollower ? (
            ...
            <p className="text-center text-gray-500 mt-4 text-lg">
              This account is private. Follow to see posts and comments.
            </p>
          )}
        </div>
      </div>
