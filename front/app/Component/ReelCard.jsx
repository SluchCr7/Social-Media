...
  // متغيرات تحديد الموضع بناءً على الاتجاه
  const infoPositionClass = isRTL ? 'right-4 md:right-6 text-right' : 'left-4 md:left-6 text-left';
  const actionsPositionClass = isRTL ? 'left-3 sm:left-5' : 'right-3 sm:right-5';
  const controlsPositionClass = isRTL ? 'left-3 sm:left-5' : 'right-3 sm:right-5';
  const userFlexDirection = isRTL ? 'flex-row-reverse' : '';
  const reshareIconPosition = isRTL ? '-left-2' : '-right-2';
...
      {/* 📄 Bottom Info */}
      <div className={`absolute bottom-5 ${infoPositionClass} text-white max-w-[80%] sm:max-w-[65%] md:max-w-[55%]`}>
        {/* 🔹 صور المالك الأصلي + من أعاد النشر */}
        <div className={`relative flex items-center gap-3 sm:gap-4 ${userFlexDirection}`}>
          <div className="relative">
            {/* صورة المالك الأصلي */}
            <img
              src={reel?.originalPost?.owner?.profilePhoto?.url || reel?.owner?.profilePhoto?.url}
...
                src={reel?.owner?.profilePhoto?.url}
                alt={reel?.owner?.username}
                className={`w-6 h-6 sm:w-7 sm:h-7 rounded-full border-2 border-white object-cover absolute -bottom-1 ${reshareIconPosition} bg-gray-800 shadow-md`}
              />
            )}
          </div>
...
      {/* 🎛️ Side-side actions */}
      <div className={`absolute ${actionsPositionClass} bottom-20 flex flex-col gap-5 sm:gap-6 text-white items-center`}>
        {/* 👁 Views */}
        <div className="flex flex-col items-center text-gray-300">
          <FaEye />
          <span className="text-[10px] sm:text-xs">{reel?.views?.length || 0}</span>
...
      {/* 🔊 Controls (Mute / Add Reel) */}
      <div className={`absolute top-4 ${controlsPositionClass} flex items-center gap-2 sm:gap-4`}>
        <button
          onClick={toggleMute}
          className="bg-black/40 p-2 sm:p-3 rounded-full text-white hover:bg-black/60 transition"
        >
