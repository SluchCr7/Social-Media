...
const SluchitEntry = ({ post }) => {
  const { likePost, savePost, sharePost, imageView, setImageView } = usePost()
  const [showMenu, setShowMenu] = useState(false)
  const { user, isLogin } = useAuth()
  ...
  return (
    <div className="bg-white/70 dark:bg-darkMode-card/60 
      backdrop-blur-xl border border-gray-200 dark:border-gray-700 
      w-full p-6 rounded-2xl flex flex-col gap-5 
      ...
      {isShared && (
        <div className='text-sm text-gray-900 dark:text-gray-300 italic mb-2'>
          <Link
            href={`/Pages/User/${post?.owner?._id}`}
            className='text-lightMode-fg dark:text-darkMode-fg font-semibold hover:underline'
          >
            {post?.owner?.username}
          </Link>{' '}
          shared a post from{' '}
          <Link
            href={`/Pages/User/${original?.owner?._id}`}
            className='text-lightMode-fg dark:text-darkMode-fg font-semibold hover:underline'
          >
            {original?.owner?.username}
          </Link>
        ...
        <div className='flex flex-col w-full gap-3'>
          ...
          <div className="flex justify-between items-center w-full">
            <div className="flex flex-col leading-tight">
              ...
              <div className="flex items-center gap-2 text-xs text-gray-500">
                <span>{isCommunityPost ? `@${post?.owner?.username}` : post?.owner?.profileName}</span>
                <span className="w-1 h-1 bg-gray-400 rounded-full" />
          ...
          {post?.Hashtags?.length > 0 && (
            <div className='flex flex-wrap gap-2'>
              {post?.Hashtags.map((tag, i) => (
                <span key={i} className='bg-gradient-to-r from-blue-500/20 to-purple-500/20 
                  text-blue-600 dark:text-blue-300 text-xs font-semibold px-3 py-1 
          ...
          {post?.Photos?.length > 0 && (
            <div className={`grid gap-2 ${post?.Photos.length > 1 ? 'grid-cols-2 sm:grid-cols-3' : ''}`}>
              {post?.Photos.map((photo, i) => (
                <div
                  key={i}
                  onClick={() => setImageView({ url: photo?.url, postId: post?._id })}
          ...
          {isLogin && (
            <div className='flex items-center gap-8 pt-4'>
              {/* Like */}
              <button onClick={() => likePost(post?._id, post?.owner._id)} className="flex items-center gap-2 group">
                {post?.likes?.includes(user?._id) ? (
                  <IoIosHeart className='text-red-500 text-2xl group-hover:scale-110 transition' />
                ) : (
                  <CiHeart className='text-gray-500 text-2xl group-hover:scale-110 transition' />
                )}
              ...
              {/* Comments */}
              {!post?.isCommentOff && (
                <Link href={`/Pages/Post/${post?._id}`} className='flex items-center gap-2 group'>
                  <FaRegCommentDots className='text-gray-500 text-xl group-hover:scale-110 transition' />
                  <span className='text-gray-600 dark:text-gray-300 text-sm font-medium'>
              ...
              {/* Share */}
              <button onClick={() => sharePost(post?.originalPost ? post?.originalPost?._id : post?._id)} className='flex items-center gap-2 group'>
                <IoIosShareAlt className='text-gray-500 text-2xl group-hover:scale-110 transition' />
                <span className='text-gray-600 dark:text-gray-300 text-sm font-medium'>
                  {post?.shares?.length}
                </span>
              </button>
              ...
              {/* Save */}
              <button onClick={() => savePost(post?._id)} className='flex items-center gap-2 group'>
                <CiBookmark className={`${post?.saved?.includes(user?._id) ? 'text-yellow-400' : 'text-gray-500'} text-2xl group-hover:scale-110 transition`} />
                <span className='text-gray-600 dark:text-gray-300 text-sm font-medium'>
                  {post?.saved?.length}
                </span>
              </button>
              ...
              <div className='flex -space-x-2'>
                {post?.comments?.slice(0, 3).map((comment, i) => (
                  <Image
                    ...
                    width={28}
                    height={28}
                    className='rounded-full border-2 border-white dark:border-black w-7 h-7 object-cover'
                  />
                ))}
              </div>
              <span className='text-gray-500 text-xs'>{post?.comments?.length} comments</span>
