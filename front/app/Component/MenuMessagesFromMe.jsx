...
const MenuMessagesFromMe = () => {
  ...
  return (
    <div className="w-full flex bg-lightMode-menu dark:bg-darkMode-menu flex-col gap-3 rounded-lg py-3 shadow-lg">
      ...
      <ul className="flex flex-col w-full text-sm text-text px-4 py-2 gap-4 max-h-60 overflow-y-auto">
        {recentMessages.length > 0 ? (
          recentMessages.map((msg, index) => (
            <Link href={`/Pages/Messanger`} key={index} className="flex items-start gap-3 w-full bg-darkMode-bgSecondary p-3 rounded-lg hover:bg-darkMode-bgHover transition">
              <Image
