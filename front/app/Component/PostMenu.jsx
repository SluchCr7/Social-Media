...
const MenuOption = ({ icon, text, action, className, loading }) => (
  <motion.button
    ...
    onClick={action}
    disabled={loading}
    className={`flex items-center gap-3 px-4 py-3 text-sm font-medium text-left transition rounded-md focus:outline-none focus:ring-2 focus:ring-blue-400
      ${className || 'text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800'}
      ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
    role="menuitem"
  >
...
const PostMenu = ({ showMenu, setShowMenu, post,triggerRef  }) => {
  ...
  useEffect(() => {
    if (showMenu && triggerRef.current) {
      ...
      const left = isRTL
        ? rect.left + scrollX
        : rect.right + scrollX - 256; // 256px = عرض القائمة
      setMenuPos({ top, left });
    }
  }, [showMenu, isRTL]);
  ...
  const ownerOptions = useMemo(() => [
    {
      icon: <CiMapPin size={18} />,
      text: userData?.pinsPosts?.some((p) => p?.id === post?._id)
        ? t('Unpin Post')
        : t('Pin Post'),
      action: async () => {
        setLoadingBtn(true);
  ...
  const menuContent = showMenu ? (
    <AnimatePresence>
      <motion.div
        ...
        exit={{ opacity: 0, scale: 0.95, y: -8 }}
        transition={{ duration: 0.2 }}
        className={`fixed w-64 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 
                    rounded-xl shadow-2xl z-[9999] ${isRTL ? 'left-auto' : 'right-auto'}`}
        style={{
          top: menuPos.top,
          left: menuPos.left,
        }}
