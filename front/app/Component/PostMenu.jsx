...
const PostMenu = ({ showMenu, setShowMenu, post }) => {
  ...
  const ownerOptions = useMemo(() => [
    {
      icon: <CiMapPin size={18} />,
      text: user?.pinsPosts?.some((p) => p?.id === post?._id) ? t('Unpin Post') : t('Pin Post'),
      action: async () => {
        setLoadingBtn(true);
        await pinPost(post?._id);
        setLoadingBtn(false);
  ...
  const visitorOptions = useMemo(() => [
    {
      icon: user?.BlockedNotificationFromUsers?.some((u) => u._id === post?.owner?._id)
        ? <GoUnmute className="text-lg" />
        : <IoVolumeMute className="text-lg" />,
      text: user?.BlockedNotificationFromUsers?.some((u) => u._id === post?.owner?._id)
        ? t('Unmute Notifications From User')
        : t('Mute Notifications From User'),
      action: async () => {
        setLoadingBtn(true);
        await toggleBlockNotification(post?.owner?._id);
        setLoadingBtn(false);
      },
      className: user?.BlockedNotificationFromUsers?.some((u) => u._id === post?.owner?._id)
        ? 'text-blue-400 hover:bg-blue-100'
        : 'text-red-400 hover:bg-red-100',
    },
    {
      ...
      action: async () => {
        setLoadingBtn(true);
        await followUser(post?.owner?._id);
        setLoadingBtn(false);
      },
      className: user?.following?.some(f => f._id === post?.owner?._id)
        ? 'text-red-600 hover:bg-red-100'
              ...
              {optionsToShow.map((option, index) => (
                <MenuOption
                  ...
                  action={async () => {
                    ...
                    await option.action();
                    setLoadingBtn(false);
                    // ⏳ انتظر قليلاً لتحديث الحالة في AuthContext قبل إغلاق القائمة
